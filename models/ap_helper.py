...
from box_util import extract_pc_in_box3d
...
def parse_predictions(end_points, config_dict):
    ...
    pred_center_upright_camera = pred_center.detach().cpu().numpy() #B,num_prop,3

    pred_tracking = np.zeros((bsize,num_proposal,7)) #3+3+1
    pred_tracking[:,:,:3] = pred_center_upright_camera[:,:,:]
    pred_cls = pred_sem_cls.reshape(bsize,num_proposal,1)

    for i in range(bsize):
        for j in range(num_proposal):
            heading_angle = np.pi/6 * (pred_heading_class[i,j].detach().cpu().numpy() - 6) + np.pi/12 + pred_heading_residual[i,j].cpu().detach().numpy() * (np.pi/12)
            box_size = MEAN_SIZE[pred_size_class[i,j].detach().cpu().numpy()] + pred_size_residual[i,j].detach().cpu().numpy()
            corners_3d_upright_camera = get_3d_box(box_size, heading_angle, pred_center_upright_camera[i,j,:])
            pred_tracking[i,j,3:6] = box_size
            pred_tracking[i, j, 6] = heading_angle
    ...
    if not config_dict['use_3d_nms']:
        ...
        for i in range(bsize):
            ...
            assert(len(pick)>0)
            pred_mask[i, nonempty_box_inds[pick]] = 1
        end_points['pred_mask'] = pred_mask #B,K
        # ---------- NMS output: pred_mask in (B,K) -----------
        ...
        if config_dict['per_class_proposal']:
            ...
            for j in range(pred_center.shape[1]):
                if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']:
                    pred_tracking_out.append(pred_tracking[i,j,:])
                    pred_cls_out.append(pred_cls[i,j,0])

    pred_tracking_out = torch.tensor(pred_tracking_out).cpu().detach().numpy().reshape(bsize,-1,7)
    pred_cls_out = torch.tensor(pred_cls_out).cpu().detach().numpy().reshape(bsize,-1,1)
    return batch_pred_map_cls, pred_tracking_out, pred_cls_out
...
def parse_groundtruths(end_points, config_dict):
    ...
    gt_ls = []
    gt_array = np.zeros((bsize,K2,8)) # center + size +angle
    gt_array[:, :, :3] = gt_center_upright_camera[:, :, :]
    for i in range(bsize):
        for j in range(K2):
            if box_label_mask[i,j] == 0: continue
            heading_angle = np.pi / 6 * (heading_class_label[i, j].detach().cpu().numpy() - 6) + np.pi / 12 + \
                            heading_residual_label[i, j].cpu().detach().numpy() * (np.pi / 12)
            box_size = MEAN_SIZE[size_class_label[i, j].detach().cpu().numpy()] + size_residual_label[
                i, j].detach().cpu().numpy()
            corners_3d_upright_camera = get_3d_box(box_size, heading_angle, gt_center_upright_camera[i,j,:])
            gt_array[i,j,3:6] = box_size
            gt_array[i, j, 6] = heading_angle
            gt_array[i,j,7] = sem_cls_label[i, j]
    ...
    for i in range(bsize):
        ...
        for j in range(gt_corners_3d_upright_camera.shape[1]):
            if box_label_mask[i, j] == 1:
                gt_ls.append(gt_array[i,j,:])

    end_points['batch_gt_map_cls'] = batch_gt_map_cls
    gt_out = torch.tensor(gt_ls).cpu().detach().numpy().reshape(bsize, -1, 8)
    return batch_gt_map_cls,gt_out
